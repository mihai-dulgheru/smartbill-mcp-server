...
export const productSchema = z.object({
  ...
  code: z
    ...
    .optional()
    .describe(
      'SKU. Required if your company has the "Foloseste cod produs" setting enabled in SmartBill Cloud, otherwise optional.',
    ),
  ...
  numberOfItems: z
    ...
    .int()
    .optional()
    .describe('How many preceding lines the discount applies to.'),
  discountType: z
    .union([z.literal(1), z.literal(2)])
    .optional()
    .describe(
      'Discount type: 1 = value-based (paired with discountValue, which must be negative); 2 = percentage (paired with discountPercentage). Defaults to 2 when omitted on a discount line. Sending any other value is not rejected but produces a wrong document.',
    ),
  discountValue: z
    .number()
    .lt(0)
    .optional()
    .describe(
      'Value-based discount amount when discountType is 1. Must be strictly negative (e.g. -10 for a 10 RON discount). A positive value is not rejected by the API but will increase the total instead of reducing it.',
    ),
  discountPercentage: z
    ...
    .optional()
    .describe(
      'Percentage discount when discountType is 2. Must be between 0 (exclusive) and 100 (inclusive). Sent as a bare number (e.g. 10, never "10%").',
    ),
...
export const invoicePaymentSchema = z.object({
  ...
  paymentSeries: z
    .string()
    .optional()
    .describe('Receipt series, required when type is "Chitanta".'),
...
const documentBase = {
  ...
  paymentUrl: z
    .string()
    .optional()
    .describe('Set to "Generate URL" to get a payment link in the response `url` field.'),
...
export const invoiceRequestSchema = z.object({
  ...
  useEstimateDetails: z
    .boolean()
    .optional()
    .describe('Copy the products from a proforma named in `estimate`.'),
  estimate: z
    .object({
      seriesName: z.string(),
      number: z.string(),
      useStock: z
        .boolean()
        .optional()
        .describe('Discharge stock when issuing the invoice generated from the proforma.'),
    })
    .optional()
...
export const paymentRequestSchema = z.object({
  ...
  currency: z
    ...
    .optional()
    .describe(
      'Payment currency. Defaults to RON. Fiscal receipts (bon fiscal) can only be issued in RON.',
    ),
  exchangeRate: z.number().optional(),
  precision: z.number().int().optional(),
  isCash: z.boolean().optional(),
  ...
  useInvoiceDetails: z
    .boolean()
    .optional()
    .describe('Take the client and products from the invoices in invoicesList.'),
  invoicesList: z
    .union([
...
export const paginationSchema = z.object({
  ...
  after: z
    .string()
    .optional()
    .describe('Cursor id to continue after. Mutually exclusive with `before`.'),
  before: z
    .string()
    .optional()
    .describe('Cursor id to continue before. Mutually exclusive with `after`.'),
});
